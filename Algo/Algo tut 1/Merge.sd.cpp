...
void mergeSort3Way(vector<int>&gArray,int 0, int n)
{
...
bool isSuccessful(vector<int>&inputs,vector<int>&insertionRes,vector<int>&mergeRes)
{
    bool allright=true;
...
int main()
{
    ...
    for(int cas=1; cas<=20; cas++)
    {
        ...
        cout<<"Case#"<<cas<<": ";
        if(isSuccessful(inputs,insertionRes,mergeRes))
        {
            cout<<"Passed!!!"<<endl;
            cout<<(second-first)<<" "<<(third-second)<<" "<<"ms"<<endl;
            successful++;
        }
        else
        {
