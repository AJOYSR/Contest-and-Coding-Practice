...
bool kt = true;
long double Get(ll A,ll B,ll C)
{
    ll Delta = B * B - 4 * A * C;
    if (Delta < 0)
    {
    ...
    if (Delta == 0)
    {
        return (long double)B / 2;
    }
    long double VC = sqrt((long double)Delta);
    long double X = B;
    return (X + VC) / 2;
}
int main()
{
    ...
    w(t)
    {
        ...
        {
            cout << "Y" << " " << fixed << setprecision(10) <<  GT << " " << (long double)D - GT << '\n';
        }
    }
}
